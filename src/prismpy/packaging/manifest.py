...
UC_CONFIG_KEY_TABLE: Dict[str, Tuple[str, ...]] = {
    "yield_forecast": (
        ...
        "observed_yields",
        # UC1-FORECAST §12 PR-6: the seasonal-predictor selector/override list (the
        # two-wire ``predictors`` param — a bare catalog id, or an ``id=path`` data
        # override). Bakeable so the manifest-field tier of the prism-runner two-wire
        # precedence survives the canonical emit (same allowlist rationale as
        # observed_yields — an unregistered baked key is silently filtered on emit).
        "predictors",
    ),
