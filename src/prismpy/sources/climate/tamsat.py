...
Cache-isolation primitives (manifest writer, marker writer, lock path,
atomic-write, bbox helpers) live in ``prismpy.sources._cache_base``
post-Sprint-G AC-G-2.0 extraction. This module re-exports the public
names for backward compatibility with the V2-22a B2 import surface
that ``agera5.py`` and existing tests depend on, plus the Region-coupled
``cache_lock_path`` wrapper that pre-converts a ``Region`` dataclass to
the generic ``key: str`` the base module expects.
"""
...
import logging
import os
import tempfile  # exposed for test monkey-patching of `tmod.tempfile.*`
import threading
...
from prismpy.sources._cache_base import (
    ...
    bbox_field_for_log,
    bbox_to_dict,
    cache_lock_path as _cache_lock_path_generic,
...
def cache_lock_path(cache_dir: Path, source: str, region_name) -> Path:
    """Region-coupled compatibility wrapper for the V2-22a B2 surface.

    Pre-converts a ``Region`` dataclass to the generic ``key: str`` that
    ``prismpy.sources._cache_base.cache_lock_path`` expects, then
    returns its result. Existing TAMSAT/AgERA5 callers (and tests)
    continue to import this name from ``tamsat.py`` unchanged; ISIMIP3b
    (Sprint G) imports the generic from ``_cache_base`` directly with
    its own per-(product/scenario/gcm/variable/bbox) string key.

    ``region_name`` must be a ``Region`` dataclass (post-resolution).
    Every live tamsat/agera5 caller passes a ``Region``; the parameter
    keeps the ``region_name`` label for call-site symmetry. Any
    non-``Region`` shape fails loudly inside
    ``region_cache_key_from_region`` at ``getattr`` on
    ``.boundary_source`` / ``.bounds``.
    """
    from prismpy.utils.sanitization import region_cache_key_from_region

    safe = region_cache_key_from_region(region_name)
    return _cache_lock_path_generic(cache_dir, source, safe)
