...
class AceaTranslator(AceaTranslatorBase):
    ...
    def _generate_soil_netcdf_from_profiles(
        ...
        # `with` wrap releases the handle on every exit path. Pin
        # DP-1 enforces this on every ``netCDF4.Dataset(...)`` site
        # under prismpy/src/ (same handle-leak class as AgERA5).
    ...
    def _generate_acea_soil_netcdf(
        ...
        # ``_generate_soil_netcdf_from_profiles`` above.
        with nc.Dataset(nc_path, 'w', format='NETCDF4') as ds:
            # Create dimensions
            ds.createDimension('lat', 360)
            ds.createDimension('lon', 720)
